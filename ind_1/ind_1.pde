...
void drawCircleAndTangents() {
  ...
  for (int i = 0; i < numTangents; i++) {
    ...
    // Малюємо точки A, L, P
    fill(255, 0, 0);
    ellipse(A.x, A.y, 10, 10);  // Точка A
    
    fill(0, 255, 0);
    ellipse(L.x, L.y, 10, 10);  // Точка L
    
    fill(255, 0, 255);
    ellipse(P.x, P.y, 10, 10);  // Точка P
    ...
    if (abs(A.x) > width / 2 || abs(A.y) > height / 2) {
      ...
      break;  // Припинити цикл, якщо точка A виходить за межі
    }
  }
}

PVector findTangentPoint(PVector A) {
  // Вектор від центру кола (0, 0) до точки A
  PVector CA = A.copy();
  float d = CA.mag();  // Відстань від центру до точки A
  
  // Перевіряємо, чи знаходиться точка A поза колом
  if (d <= rCircle) {
  ...
  // Обчислюємо кут для точки дотику
